...
instance (Pretty r, Pretty m) => Pretty (Expr r t m) where
  ...
  pretty (Foreign _ BidirectionalChannel _) = text "bidirectional_channel"
  pretty (Foreign _ Receiver _) = text "receiver"
  pretty (Foreign _ Sender _) = text "sender"
  pretty (Foreign _ (ForeignSymbol s) _) = pretty s
  pretty (Go _ expr _) = text "go" <+> pretty expr
  pretty (Send _ receiver value _) = pretty receiver <+> text "<-" <+> pretty value
