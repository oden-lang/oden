...
(define compiler-tests
  (test-suite
   ...
   (test-case
    "partial application"
    (check-equal?
     (compile-single-expression '(((lambda (x y) x) 1) 2))
     "(func (x int) func (int) (int) {\nreturn (func (y int) int {\nreturn x\n})\n})(1)(2)"))

   (test-case
    "let"
    (check-equal?
     (compile-single-expression '(let ([x 1]) (+ x 2)))
     "(func () int {\nvar x int = 1\nreturn (x + 2)\n}())"))

   (test-case
   "let type annotated"
    (check-equal?
     (compile-single-expression '(let ([[x : int] 1]) (+ x 2)))
     "(func () int {\nvar x int = 1\nreturn (x + 2)\n}())"))
      
   (test-case
    "higher-order functions"
    (check-equal?
     (compile-single-expression '(((lambda (x y) (x y)) (lambda (x) x)) 1))
     "(func (x func (int) (int)) func (int) (int) {\nreturn (func (y int) int {\nreturn x(y)\n})\n})((func (x int) int {\nreturn x\n}))(1)"))

   (test-case
    "call by name"
    (check-equal?
     (compile-single-expression '(let ([make-num (lambda () 3)]) (* (make-num) (make-num))))
